...
public class RequestParser {
    ...
    enum State {
        ...
        BODY(p -> p.tokenizer.next(CRLF), (rp, token) -> rp.parseBody(token)),
        CHUNK_SIZE(p -> p.tokenizer.next(CRLF), (rp, token) -> rp.parseChunkSize(token)),
        CHUNK_DATA(p -> p.tokenizer.next(CRLF), (rp, token) -> rp.parseChunkData(token)),
        CHUNK_DATA_END(p -> p.tokenizer.next(CRLF), (rp, token) -> rp.parseChunkDataEnd()),
    ...
    private void parseChunkSize(byte[] token) {
        ...
        state = chunkSize == 0 ? State.CHUNK_TRAILER : State.CHUNK_DATA;
    }
    ...
    private void parseChunkDataEnd() {
        state = State.CHUNK_SIZE;
    }
    ...
    private OptionalInt findContentLength() {
        try {
            return headers.stream()
                    .filter(h -> h.name().equalsIgnoreCase(HEADER_CONTENT_LENGHT))
                    .map(Header::value)
